...
covered by the existing read-only bind mount; no extra mount is required.
...
- **Deployment**: the backend service runs as the host user (`NANOCLAW_UID`/`NANOCLAW_GID` in compose) so `backups/` archives + restore scripts are owned by the operator, not root.
